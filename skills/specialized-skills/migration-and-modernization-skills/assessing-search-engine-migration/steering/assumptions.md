...
When the user can't supply a value, fall back to these defaults — but always state the default in
the report and flag that the user should validate against their actual environment.
...
| `pricing.discount_factor` | 1.0 (no discount) | Always present list-price first; discounts are user-specific. |
...
If the user's value differs by more than 2× from a default, treat that as a strong signal that the
recommendation must be re-derived rather than nudged from the default — recompute from scratch.
